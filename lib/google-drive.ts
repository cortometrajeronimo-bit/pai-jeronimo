// Helper de Google Drive API v3 con Service Account (sin OAuth flow).
// Free tier: 1 millón de queries/día, 1000 queries/100s/usuario — más que suficiente.
//
// Configuración requerida en env:
//   GOOGLE_SERVICE_ACCOUNT_EMAIL   email del service account
//   GOOGLE_SERVICE_ACCOUNT_KEY     private_key del JSON (con \n literales OK)
//   GOOGLE_DRIVE_ROOT_FOLDER_ID    ID de la carpeta raíz del proyecto
//
// Compartir la carpeta de Drive con el email del service account (modo lector).
//
// Si faltan credenciales, todas las funciones retornan datos mock para no romper UI.

import crypto from "crypto";

const TOKEN_URL = "https://oauth2.googleapis.com/token";
const DRIVE_API = "https://www.googleapis.com/drive/v3";
const DOCS_API = "https://docs.googleapis.com/v1";
const SHEETS_API = "https://sheets.googleapis.com/v4";

const SCOPES = [
  "https://www.googleapis.com/auth/drive.readonly",
  "https://www.googleapis.com/auth/documents.readonly",
  "https://www.googleapis.com/auth/spreadsheets.readonly",
].join(" ");

export type DriveFileLite = {
  id: string;
  name: string;
  mimeType: string;
  webViewLink?: string;
  modifiedTime?: string;
  size?: string;
};

// Cache de access token en memoria de la lambda (válido 1h)
let cachedToken: { token: string; expiresAt: number } | null = null;

export function driveDisponible(): boolean {
  return !!(
    process.env.GOOGLE_SERVICE_ACCOUNT_EMAIL &&
    process.env.GOOGLE_SERVICE_ACCOUNT_KEY &&
    process.env.GOOGLE_DRIVE_ROOT_FOLDER_ID
  );
}

// ==================================================
// JWT manual con RS256 (sin dependencias extra)
// ==================================================
function base64url(input: Buffer | string): string {
  const buf = typeof input === "string" ? Buffer.from(input) : input;
  return buf
    .toString("base64")
    .replace(/=/g, "")
    .replace(/\+/g, "-")
    .replace(/\//g, "_");
}

async function obtenerAccessToken(): Promise<string> {
  if (cachedToken && cachedToken.expiresAt > Date.now() + 60_000) {
    return cachedToken.token;
  }

  const email = process.env.GOOGLE_SERVICE_ACCOUNT_EMAIL!;
  const privateKey = (process.env.GOOGLE_SERVICE_ACCOUNT_KEY ?? "").replace(/\\n/g, "\n");

  const now = Math.floor(Date.now() / 1000);
  const header = { alg: "RS256", typ: "JWT" };
  const claim = {
    iss: email,
    scope: SCOPES,
    aud: TOKEN_URL,
    exp: now + 3600,
    iat: now,
  };

  const headerB64 = base64url(JSON.stringify(header));
  const claimB64 = base64url(JSON.stringify(claim));
  const dataParaFirmar = `${headerB64}.${claimB64}`;

  const sign = crypto.createSign("RSA-SHA256");
  sign.update(dataParaFirmar);
  sign.end();
  const firma = base64url(sign.sign(privateKey));
  const jwt = `${dataParaFirmar}.${firma}`;

  const r = await fetch(TOKEN_URL, {
    method: "POST",
    headers: { "Content-Type": "application/x-www-form-urlencoded" },
    body: new URLSearchParams({
      grant_type: "urn:ietf:params:oauth:grant-type:jwt-bearer",
      assertion: jwt,
    }),
  });

  if (!r.ok) {
    throw new Error(`Google token error: ${r.status} ${await r.text()}`);
  }
  const data = (await r.json()) as { access_token: string; expires_in: number };
  cachedToken = {
    token: data.access_token,
    expiresAt: Date.now() + data.expires_in * 1000,
  };
  return cachedToken.token;
}

async function get(url: string): Promise<unknown> {
  const token = await obtenerAccessToken();
  const r = await fetch(url, { headers: { Authorization: `Bearer ${token}` } });
  if (!r.ok) throw new Error(`Drive API error: ${r.status} ${await r.text()}`);
  return r.json();
}

// ==================================================
// Listado de archivos en carpeta
// ==================================================
export async function listarArchivos(folderId?: string): Promise<DriveFileLite[]> {
  if (!driveDisponible()) {
    return mockArchivos();
  }
  const id = folderId ?? process.env.GOOGLE_DRIVE_ROOT_FOLDER_ID!;
  const q = encodeURIComponent(`'${id}' in parents and trashed = false`);
  const fields = encodeURIComponent("files(id,name,mimeType,webViewLink,modifiedTime,size)");
  const data = (await get(
    `${DRIVE_API}/files?q=${q}&fields=${fields}&pageSize=100&orderBy=modifiedTime desc`
  )) as { files: DriveFileLite[] };
  return data.files ?? [];
}

// ==================================================
// Leer contenido — Docs, Sheets, texto plano
// ==================================================
export async function leerContenido(
  fileId: string,
  mimeType: string
): Promise<string> {
  if (!driveDisponible()) return "";

  // Google Docs
  if (mimeType === "application/vnd.google-apps.document") {
    const data = (await get(`${DOCS_API}/documents/${fileId}`)) as {
      body?: { content?: Array<{ paragraph?: { elements?: Array<{ textRun?: { content?: string } }> } }> };
    };
    const partes: string[] = [];
    for (const elem of data.body?.content ?? []) {
      for (const p of elem.paragraph?.elements ?? []) {
        if (p.textRun?.content) partes.push(p.textRun.content);
      }
    }
    return partes.join("").trim();
  }

  // Google Sheets — concatenar todas las hojas como TSV
  if (mimeType === "application/vnd.google-apps.spreadsheet") {
    const meta = (await get(
      `${SHEETS_API}/spreadsheets/${fileId}?fields=sheets.properties.title`
    )) as { sheets: Array<{ properties: { title: string } }> };
    const nombres = meta.sheets.map((s) => s.properties.title);
    const partes: string[] = [];
    for (const nombre of nombres) {
      const range = encodeURIComponent(nombre);
      const valores = (await get(
        `${SHEETS_API}/spreadsheets/${fileId}/values/${range}`
      )) as { values?: string[][] };
      partes.push(`### Hoja: ${nombre}`);
      for (const fila of valores.values ?? []) {
        partes.push(fila.join("\t"));
      }
    }
    return partes.join("\n");
  }

  // text/plain, csv → export
  if (mimeType.startsWith("text/")) {
    const token = await obtenerAccessToken();
    const r = await fetch(`${DRIVE_API}/files/${fileId}?alt=media`, {
      headers: { Authorization: `Bearer ${token}` },
    });
    if (!r.ok) return "";
    return r.text();
  }

  // PDF, imágenes, otros → no extraemos texto (TODO: integrar OCR si necesario)
  return "";
}

// ==================================================
// Parser de Sheets a filas estructuradas (para mapear a expenses/cash_flow)
// ==================================================
export type FilaSheet = Record<string, string>;

export async function leerSheetComoFilas(fileId: string): Promise<FilaSheet[]> {
  if (!driveDisponible()) return [];
  // Tomamos solo la primera hoja para simplificar
  const meta = (await get(
    `${SHEETS_API}/spreadsheets/${fileId}?fields=sheets.properties.title`
  )) as { sheets: Array<{ properties: { title: string } }> };
  const primera = meta.sheets[0]?.properties.title;
  if (!primera) return [];

  const range = encodeURIComponent(primera);
  const valores = (await get(
    `${SHEETS_API}/spreadsheets/${fileId}/values/${range}`
  )) as { values?: string[][] };

  const filas = valores.values ?? [];
  if (filas.length < 2) return [];

  const headers = filas[0].map((h) => h.toLowerCase().trim());
  return filas.slice(1).map((fila) => {
    const obj: FilaSheet = {};
    headers.forEach((h, i) => {
      obj[h] = fila[i] ?? "";
    });
    return obj;
  });
}

// ==================================================
// Mock data (cuando no hay credenciales configuradas)
// ==================================================
function mockArchivos(): DriveFileLite[] {
  return [
    {
      id: "mock-1",
      name: "Guion JERÓNIMO v3.docx (mock)",
      mimeType: "application/vnd.google-apps.document",
      webViewLink: "#",
      modifiedTime: "2026-05-01T10:00:00Z",
    },
    {
      id: "mock-2",
      name: "Presupuesto Maestro.xlsx (mock)",
      mimeType: "application/vnd.google-apps.spreadsheet",
      webViewLink: "#",
      modifiedTime: "2026-05-05T15:30:00Z",
    },
    {
      id: "mock-3",
      name: "Carpeta — Locaciones (mock)",
      mimeType: "application/vnd.google-apps.folder",
      webViewLink: "#",
      modifiedTime: "2026-04-20T09:00:00Z",
    },
  ];
}
